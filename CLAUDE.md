...
- **Components:** `x-button` (variants incl. `inverse` for image overlays; `focus-visible`), `x-card`, `x-badge`, `x-alert`, `x-stat` (with `tone` + `active`), `x-modal` (`@entangle`), `x-dropdown` + `x-dropdown.{item,divider}`, `x-th-sort`, `x-upgrade-gate`, `x-empty-state`, `x-input.{text,email,password,textarea,select,checkbox,search}`, `x-app-logo`. Money via `Domain\Shared\Support\Currency`.
- **Actions convention (house style):** a row/section with **two actions or fewer renders them inline; three or more collapse into `x-dropdown`**. Where one action is clearly primary and high-frequency (Review a document, View an order, Approve a parsed row) it stays inline *beside* the menu. The menu positions itself `fixed` from the trigger's bounding box — absolute positioning gets clipped by the `overflow-x-auto` table wrappers — and flips above the trigger when short of room. `x-dropdown.item` renders `<a>` (`href`), `<button>` (default, for `wire:click`) or a CSRF'd POST form (`post`), and supports `variant="danger"` and `disabled`.
...
- **The Company is the tenant**, not the user. `Domain\Company\Models\Company` is the Cashier `Billable` and holds the `plan`; **users are seats** (a `role` of `owner`/`manager`/`member` — see `Domain\User\Enums\Role`), and **venues belong to the company** (`company_id`). A user's venue access is **role-aware**: owners/managers see every company venue, members only the venues assigned to them via the **`user_venue`** pivot. App code resolves this through `App\Livewire\Concerns\WithTenant` (`currentCompany()`, `companyPlan()`, `accessibleVenues()`) + `Domain\Company\Repositories\CompanyRepository::getLoggedInCompany()`. Plan gating reads the company's **effective** plan — `CompanyData::effectivePlan()`, NEVER the raw `plan` column — because a lapsed trial can reduce entitlement; billing (Pricing/checkout/portal/webhook `UpdateCompanyPlanFromStripe`) is **owner-only** and acts on the Company billable. Registration creates company→owner→venue→pivot. Owners/managers manage the team at **`/team`** (invite users by email at-or-below their own role, assign venue visibility); admins manage companies/plans/teams at **`/admin/companies`**.
- **Plan and price are separate questions.** `companies.plan` is the ENTITLEMENT ladder (what a company can do); `billing_arrangement` is what we charge for it (`standard` / `comped` = free indefinitely / `custom` = a negotiated amount in minor units). A partner on Group for nothing and a customer on Group at list price have identical capabilities and different invoices. **`Domain\Billing\Support\PlanEntitlement`** decides what is actually in force and everything gating MUST go through `CompanyData::effectivePlan()`. Its rules, all of which fail SAFE (when unsure, the company keeps what it has): only a `standard` arrangement depends on a Stripe subscription (a negotiated price isn't in the Stripe catalogue, so such a customer could never satisfy the check); a company with **no** trial is untouched, which is what made this safe to add over live data; a lapsed trial with no subscription drops to `Plan::default()`, and `has_active_subscription` is `?bool` where **null means "not established" and never downgrades**. Note a trial on the entry tier revokes nothing (there is nothing below it) — `trialLapsed()` is the commercial signal, `entitlementReduced()` is whether access actually changed, and the UI must not conflate them. Admins set all of this at `/admin/companies/{uuid}`, and provision whole tenants (company + terms + first venue + invited owner) at `/admin/companies/create` via `ProvisionCompanyAction`. **The Stripe webhook ignores non-standard arrangements** — comping a customer means cancelling their subscription, which would otherwise fire `subscription.deleted` and silently demote them — and clears `trial_ends_at` on conversion so a later declined card can't resurrect an old trial date.
...
- **Supplier portal** — a third isolated auth domain (`supplier` guard) at `/supplier`: login (throttled), dashboard, **Documents** (upload portfolios/price sheets to the private disk → status **Awaiting Analysis**; download/delete own files), company **Profile**. Uses `layouts/supplier.blade.php`. Admins provision accounts under **`/admin/suppliers`** (list/create companies → `SupplierShow`: edit profile, add/remove portal users with **email invite links** via the `supplier_users` password broker, **upload documents directly** (no portal account needed; optional published URL enrols the doc in the weekly refresh — the upload's SHA-256 is recorded so the first refresh only re-ingests on change), list documents, trigger **Analyse**, download). **Livewire 4 gotcha (fixed everywhere 2026-07-19): `TemporaryUploadedFile::store()` MOVES the temp file when target disk == temp disk — read `getClientOriginalName/getMimeType/getSize/get()` BEFORE `store()`, never after** (fake uploads in tests don't exhibit the move, so only real browser uploads break). The analysis pipeline is **fully implemented**: `AnalyseSupplierDocumentJob` (`timeout` 1800s, `tries` 1 — `DB_QUEUE_RETRY_AFTER` must exceed it) drives `AwaitingAnalysis → Analysing → Analysed | Failed`, calling `Domain\Supplier\Services\DocumentAnalysisService`, which parses the document with Claude (see **Portfolio parsing** below) into the `parsed_wines` review queue and records a summary in `analysis_notes`.
- **Catalogue** — sortable/filterable product table, inline price edit, session basket (`order-basket`) that feeds Orders.
- **Inventory** — per-venue stock (active-venue selector), quantity stepper, archive/restore, file attachments (private disk + authed download). Gated: Pro (page, manual add, archive, attachments), Group (2nd+ venue).
- **Import** — CSV/Excel → column mapping → preview → import wizard with `NormaliseService` (colour/grape/region standardisation, price/vintage/format parsing, region/country geocoding); remembers supplier mappings; idempotent upsert. **Always entered from a supplier** (`/suppliers/{uuid}/import`, `App\Livewire\Import\Index` with a `uuid` mount param that 403s for an unconnected supplier) — there is no standalone Import nav item, and the wizard has no supplier picker.
- **AI Search (catalogue)** — a plain-English search box above the filters. `ClaudeClient::interpretSearch` (structured output, all-required strings, `additionalProperties:false`) reads the sentence into the filters the catalogue ALREADY has; `Domain\Catalogue\Services\AiSearchService` caches per company+question (1h), and the result is applied to the ordinary filter properties so the buyer sees and can correct every criterion. **The model never writes a query** — the interpretation runs through the same scoped `ProductRepository::search()`, so tenant/supplier scoping is structurally out of its reach. Facets are passed **most-common-first** (`popularValues()`) so it picks "Bourgogne" over a stray "Burgundy". A reading that finds nothing is **relaxed** one criterion at a time (grape → producer → sub-type → region → vintage → search → country) and the buyer is told what was set aside — grape leads because supplier lists record it so unevenly (40 of 4,950 Bourgogne wines carry one). "Why this wine" is computed **deterministically** from the criteria each wine meets, not asked of the model, so it can never contradict the row. Throttled 20/min per company; pinned to Haiku via `services.anthropic.search_model` (~$0.0017 a search vs ~$0.012 on Opus).
...
- **LWIN reference layer** — the Liv-ex LWIN database (Creative Commons, 200k+ wines; the trade's canonical wine identifiers) lives in the `lwins` table — third-party REFERENCE data, separate from `wine_facts` and excluded from golden (re-importable from the source file kept at `storage/app/private/lwin/`). `wine:lwin-refresh {path}` imports (CSV streamed / xlsx chunked, header-driven, idempotent upsert; computes normalised `identity_key`/`name_key` via `WineIdentity`). **`wine:lwin-sync`** (scheduled weekly, Mondays 05:00 — needs the scheduler cron, on Forge via the site's Scheduler panel) downloads the published file, SHA-256-gates it, and re-imports + re-matches only on change; the xlsx path streams via XMLReader (~64s for 211k rows). `wine:lwin-match [--llm]` links `products`/`wine_facts` (`lwin` + `lwin_source` columns): deterministic identity + display-name passes only ever accept keys mapping to exactly ONE LWIN; `--llm` runs a capped Haiku residue pass where candidates share the producer (model picks or abstains; picks outside the candidate list are discarded). **Catalogue enrichment is provenance-labelled and supplier-first**: the supplier's own data always wins; gaps fill from LWIN reference first (book icon, "From the Liv-ex LWIN wine database — not provided by this supplier"), then other vendors' facts (sparkles, "another vendor's information"); contested fields withheld (`x-enriched-fact` `source` prop; `LwinRepository::forProducts`). Attribution: credit the Liv-ex LWIN database where reference data is surfaced.
- **Published-list refresh (weekly)** — `supplier_documents` carry **source tracking** (`source_url`, `content_sha256`) plus archive columns (`archived_at`, `superseded_by_document_id`). **`wine:refresh-documents [--process] [--approve] [--model=] [--only=]`** re-downloads every current source-tracked document, SHA-256-gates it, and on change records a **new edition** (dated filename) while **archiving the old one with a supersede pointer — old documents, their files and parsed_wines are kept for history, never deleted** (`SupersedeSupplierDocumentAction`). `--process` re-parses immediately (saved parse profiles make pattern/tabular re-runs ~free), `--approve` commits unflagged rows via the idempotent upsert (prices/attributes refresh in place) and then **archives the wines that dropped out** of the new edition. Products carry a listing lifecycle (`last_seen_at`, `archived_at`, `source_document_id` — provenance is environment-local, set by `ApproveParsedWineAction`): archived wines are hidden from browse/search/map/counts (`ProductRepository`) but direct id/uuid lookups still work for inventory/order references, and a wine that reappears in a later edition **un-archives automatically** via `UpsertProductAction` (which stamps `last_seen_at`). `archived_at`/`last_seen_at` travel with golden so prod mirrors the archive state; `source_document_id` deliberately does NOT (document ids are per-environment — prod has none). Magic-byte guard stops bot-block HTML pages superseding a good file. Scheduled Mondays 06:00 in `routes/console.php` (no-op on prod — no source-tracked docs there); the REAL run is the host cron `/home/dan/bin/cellaros-weekly-refresh.sh` (Mondays 06:00, flock-guarded) which refreshes locally then `wine:export-golden` + `wine:push-golden`s to prod using the token at `~/.cellaros-prod-ingestion-token` (90-day expiry — reissue on prod via `api:issue-token` when pushes 401).
- **Golden snapshots + ingestion API** — canonical trade data (PUBLIC suppliers, their catalogues, GLOBAL parse recipes, wine_facts — never tenant data) is exportable/restorable so `migrate:fresh` never costs a parse: `wine:export-golden` / `wine:import-golden` (JSON on the private disk; import order suppliers → wines → facts-exact-restore → recipes; everything `updateOrCreate`-idempotent, malformed rows skipped not fatal). The same payload format ships over HTTP via **`/api/ingest/{suppliers,wines,facts,parse-profiles,status}`** (Sanctum bearer tokens issued to ADMINS via `api:issue-token` — `ability:ingestion`, 90-day default expiry, `api:revoke-tokens`; throttled; private suppliers/company data structurally unreachable). `wine:push-golden {url}` pushes a local snapshot to a remote — documents CAN be parsed locally and pushed, but since 2026-07-19 prod also has its own LLM key + queue worker, so admin/portal/buyer uploads analyse natively there too. Import actions: `ImportListedSuppliersAction`, `ImportParseProfilesAction`, `ImportCatalogueWinesAction` (via `UpsertProductAction`, so facts contribution comes free), `ImportWineFactsAction`.
...
Plan gating: in-component (`Plan::can(Feature)`) + the `feature:<key>` route middleware (redirects to `pricing`, or the dashboard while the pricing flag is off); UI shows `x-upgrade-gate`.

**Feature flags** (`config/features.php` + the `flag:<key>` route middleware): areas that are built and tested but deliberately not exposed. Currently **`map` and `pricing` are OFF** — their routes 404, they drop out of the sidenav and guide, and every in-app link to them is guarded on the flag. Set `FEATURE_MAP=true` / `FEATURE_PRICING=true` to bring one back whole. Tests that cover a hidden area turn its flag on in `beforeEach`.
...
// In code:
$plan = (new CompanyRepository())->getLoggedInCompany()?->plan ?? Plan::default();
if ($plan->can(Feature::MultiVenue)) { /* ... */ }
```
...
it('reserves multiple venues for the Group plan', function () {
...
- `companies` — the tenant: `uuid`, `name`, `base_currency`, `plan`, + Cashier columns (`stripe_id`, `pm_type`, `pm_last_four`, `trial_ends_at`). `subscriptions.company_id` is the Cashier owner key. **Commercial terms** live alongside the plan: `billing_arrangement` (`standard|comped|custom`, cast via the forgiving `BillingArrangementCast`), `custom_price_amount` (MINOR units) + `custom_price_currency` + `custom_price_interval`, and `billing_notes`. Trials reuse Cashier's `trial_ends_at`.
...
- `suppliers` — wine companies; profile columns (`address`/`city`/`postcode`/`country`/`website`); **`type_mapping`** JSON (their wine-type word => our type/sub-type, a null type meaning "seen but not yet placed"); **tier** columns `created_by_company_id` (FK companies, cascadeOnDelete — set = Private) and `onboarded_at` (set = Onboarded). Tier derived in `SupplierData->tier`.
...
- `products` — full wine attributes; `grape` is JSON; **`colour` cast to `WineType`** (the COLUMN keeps the old name deliberately — renaming it would break golden payloads, saved parse recipes and prod at once; the domain language is "type"), optional **`sub_type`** cast to `WineSubType` (sparkling/fortified styles only), **`price_state`** cast to `PriceState` (`priced|poa|tbc`) + **`price_note`** carrying the supplier's own "ask us" wording; geo `latitude`/`longitude`; indexes on `(country, region)`, `colour`, `(colour, sub_type)` and `price_state`. **Natural identity for the idempotent `UpsertProductAction` is `supplier_id + wine_name + producer + vintage + format_ml`** — producer is part of it because a bare varietal name ("Chardonnay", "Riesling") is shared by many growers, so keying without producer collapsed distinct wines into one row (fixed 2026-07-20; note MySQL's default collation is case-INSENSITIVE, so "PINOT NOIR" and "Pinot Noir" match). On update the action is **fill-don't-overwrite for descriptive attributes** (`country/region/sub_region/grape/colour/lat/lng`): a null incoming value means "this source didn't say", so an existing value is kept, never blanked — prices/stock/lifecycle always take the new value. **Golden/prod caveat:** changing the identity key means a golden push can leave a few orphan rows on prod where it had previously collapsed a collision — reconcile by SSH after the first push (see [[golden-push-rename-gotcha]]).
...
- `orders` / `order_items` — `status` cast to `OrderStatus`; ordering is **unit-based** (`quantity_units`, 1 unit = 1 bottle); `unit_price_at_order` / `currency_at_order` snapshot the price at order time. **`unit_price_at_order` is NULLABLE: null means POA** — the line prints as POA and contributes nothing to the total.
