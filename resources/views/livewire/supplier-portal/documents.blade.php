<div class="space-y-6">
    <div>
        <h2 class="font-serif text-2xl font-semibold">Documents</h2>
        <p class="mt-1 text-sm text-muted-foreground">Upload portfolios and price sheets. Each is stored securely and queued for analysis.</p>
    </div>

    <x-card title="Upload a document">
        <form wire:submit="upload" class="space-y-4">
            <x-input.text name="title" label="Title (optional)" wire:model="title" placeholder="e.g. Spring 2026 portfolio" />
    ...
    @if($documents->isEmpty())
        <x-card><x-empty-state icon="file-text" title="No documents yet" message="Upload your first portfolio above to get started." /></x-card>
    @else
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-border">
                    @foreach($documents as $document)
                        <tr wire:key="doc-{{ $document->id }}" class="hover:bg-accent/40">
                            ...
                            <td class="px-3 py-2.5 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <x-button :href="route('supplier.documents.download', $document->id)" variant="ghost" size="sm" aria-label="Download">
                                        <x-icon.download class="size-4" />
                                    </x-button>
                                    <x-button wire:click="delete({{ $document->id }})" wire:confirm="Remove this document?" variant="ghost" size="sm" class="text-destructive hover:bg-destructive/10" aria-label="Delete">
